...
//TOMAS import Lomiri.Launcher 0.1
...
StyledItem {
    ...
    function _onMainAppChanged(appId) {
        ...
        if (appId !== "") {
            ...
            //TOMAS launcher.hide(launcher.ignoreHideIfMouseOverLauncher);
        }
    ...
    property bool oxLaunched: false
    
    Component.onCompleted: {
        // Launch OxLauncher once the shell is ready
        if (!oxLaunched) {
            oxLaunched = true;
            Qt.openUrlExternally("application:///tomatosoft.oxlauncher_oxlauncher_1.0.0.desktop")
        }
    ...
    AvailableDesktopArea {
        ...
        anchors.leftMargin: 0 //TOMAS (launcher.lockedByUser && launcher.lockAllowed) ? launcher.panelWidth : 0
    }
    ...
    Item {
        ...
        Stage {
            ...
            dragAreaWidth: 0 //TOMAS shell.edgeSize
            ...
            //TOMAS launcherLeftMargin: launcher.visibleWidth
                ...
                if (status !== TouchGestureArea.Recognized) {
                    if (status === TouchGestureArea.WaitingForTouch) {
                        if (wasPressed && !dragging) {
                            //TOMAS launcher.toggleDrawer(true);
                        }
                    }
                    wasPressed = false;
                }
    ...
    InputMethod {
        ...
        anchors {
            ...
            leftMargin: 0 //TOMAS (launcher.lockedByUser && launcher.lockAllowed) ? launcher.panelWidth : 0
        }
        z: notifications.useModal || panel.indicators.shown || wizard.active || tutorial.running ? overlay.z + 1 : overlay.z - 1
           //TOMAS || launcher.drawerShown
    }
        ...
        Connections {
            ...
            function onActiveChanged() {
                ...
                if (greeterLoader.toggleDrawerAfterUnlock) {
                    //TOMAS launcher.toggleDrawer(false);
    ...
    Component {
        ...
        Greeter {
            ...
            hides: [/*TOMAS launcher,*/ panel.indicators, panel.applicationMenus]
            ...
            //        left-edge drag
            dragHandleLeftMargin: 0 //TOMAS launcher.available ? launcher.dragAreaWidth + 1 : 0

            onTease: {
                //TOMAS if (!tutorial.running) {
                //     launcher.tease();
                // }
            }
    ...
    Component {
        id: secondaryGreeter
        SecondaryGreeter {
            hides: [/*TOMAS launcher,*/ panel.indicators]
        }
    }
    ...
    Item {
        ...
        Panel {
            ...
            applicationMenuContentX: 0 //TOMAS launcher.lockedVisible ? launcher.panelWidth : 0

            indicators {
                hides: [] //TOMAS [launcher]
            ...
            readonly property bool focusedSurfaceIsFullscreen: shell.topLevelSurfaceList.focusedWindow
                ...
                : false
            fullscreenMode: (focusedSurfaceIsFullscreen && !LightDMService.greeter.active && /*TOMAS launcher.progress == 0 &&*/ !stage.spreadShown)
        ...
        KeyboardShortcutsOverlay {
            ...
            enabled: width < parent.width - padding && height < parent.height - padding - panel.panelHeight
            //TOMAS enabled: launcher.shortcutHintsShown && width < parent.width - (launcher.lockedVisible ? launcher.panelWidth : 0) - padding
            //          && height < parent.height - padding - panel.panelHeight
            anchors.centerIn: parent
            anchors.horizontalCenterOffset: 0 //TOMAS launcher.lockedVisible ? launcher.panelWidth/2 : 0
        ...
        Tutorial {
            ...
            delayed: dialogs.hasActiveDialog || notifications.hasNotification ||
                     inputMethod.visible ||
                     //TOMAS (launcher.shown && !launcher.lockedVisible) ||
    ...
    Cursor {
        ...
        onPushedLeftBoundary: {
            if (buttons === Qt.NoButton) {
                //TOMAS launcher.pushEdge(amount);
